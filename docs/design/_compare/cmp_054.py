...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8000/fachkunde/glossar", wait_until="networkidle")
    page.wait_for_timeout(400)
    clip = page.evaluate(
        """() => {
          const r = document.querySelector('.app-frame').getBoundingClientRect();
          return {x:r.x,y:r.y,width:r.width,height:Math.min(r.height,920)};
        }"""
    )
    page.screenshot(path=str(out / "live-05-4.png"), clip=clip)
    f = Image.open(out / "figma-05-4.png").convert("RGB")
    l = Image.open(out / "live-05-4.png").convert("RGB")
    f = f.resize(l.size, Image.Resampling.LANCZOS)
    d = np.abs(np.asarray(f, np.float32) - np.asarray(l, np.float32)).mean(axis=2)
    print(f"05.4 pixel_delta={(d > 30).mean() * 100:.2f}% mae={d.mean():.1f} live={l.size}")
    b.close()
